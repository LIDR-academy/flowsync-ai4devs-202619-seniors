...
#   ./publicar-cohorte.sh publicar 202610-seniors s1 end     # s1/end + s2/start
...
cmd_crear () {
  ...
  echo "    $0 publicar $et s1 end     # s1/end + s2/start"
  echo
...
cmd_publicar () {
  ...
  # sN/end ARRASTRA s(N+1)/start. Tras impartir un módulo el alumno necesita las
  # dos: la solución y la rama de partida del siguiente, que desciende de ella.
  # Por separado, la segunda se olvidaba. A partir de s5/end no hay siguiente en
  # el canónico, y entonces se publica solo la end.
  local ramas=("$rama") sig="s$((n+1))/start"
  if [ "$tipo" = "end" ]; then
    if existe "$sig"; then ramas+=("$sig")
    else echo "  (No hay «${sig}» en el canónico: se publica solo «${rama}».)"; fi
  fi

  # GUARDARRAÍL: lo que reparte una solución no se ve en el nombre de la rama.
  #   sN/end         → la solución del Módulo N (y con ella, s(N+1)/start)
  #   sN/start, N>1  → la solución del Módulo N-1, que es su base
  local modulo=""
  if [ "$tipo" = "end" ]; then modulo="$n"
  elif [ "$n" -gt 1 ]; then modulo=$((n-1)); fi
  if [ -n "$modulo" ]; then
    local lista="«${ramas[0]}»"; [ ${#ramas[@]} -gt 1 ] && lista="$lista y «${ramas[1]}»"
    ...
    if [ ${#ramas[@]} -gt 1 ]; then
      local extra; extra=$(git -C "$tmp/src" rev-list --count "refs/heads/$rama..refs/heads/$sig")
      if ! git -C "$tmp/src" merge-base --is-ancestor "refs/heads/$rama" "refs/heads/$sig"; then
        echo "    ⚠️  «${sig}» NO desciende de «${rama}»: no es la cadena habitual del curso, revísala."
      elif [ "$extra" -gt 0 ]; then
        echo "    «${sig}» trae además $extra commit(s) propios: el punto de partida del Módulo $((n+1))."
      fi
    fi
